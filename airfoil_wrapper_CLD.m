function out=airfoil_wrapper_CLD(x)
...
if status == 0
    ...
    try
        ...
        CLD_max_val = airfoil_func_calls_CLD(new_coords);
    catch ME
        disp('Error processing Python output or calling airfoil_func_calls_CLD:');
        disp(ME.message);
        CLD_max_val = 0;
        error_flag = 33;
    end
...
% fid = fopen('./debug', 'w');
% fprintf(fid, '%d %f', error_flag, CLD_max_val);
% fclose(fid);
